...
console.log("RAG Ingest Edge Function started")
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 6. Python API に転送
    console.log(`Forwarding request to: ${pythonApiUrl}/ingest`)
    
    const response = await fetch(`${pythonApiUrl}/ingest`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-User-Id': user.id,           // ユーザーIDを追加
        'X-User-Email': user.email || '', // ユーザーEmailを追加
      },
      body: requestBody
    })
